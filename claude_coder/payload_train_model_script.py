"""
payload_train_model_script.py
Continue training an existing PayloadMAE model with additional decrypted benign PCAP data.
...
Usage:
    python payload_train_model_script.py <model_name> <more_benign_decrypted.pcap> [options]

Examples:
    python payload_train_model_script.py payload_baseline more_clean.pcap
    python payload_train_model_script.py payload_baseline more_clean.pcap --epochs 15 --output-name payload_baseline_v2
"""
...
def main():
    parser = argparse.ArgumentParser(
        description='Continue training an existing PayloadMAE model with additional decrypted PCAP data'
    )
    parser.add_argument(
        'model_name',
    ...
    parser.add_argument(
        'pcap_file',
        help='Path to decrypted PCAP file with additional benign/normal traffic'
    )
    parser.add_argument(
        '--epochs',
        type=int,
    ...
    print("=" * 70)
    print("PayloadMAE — Continue Training")
    print("=" * 70)
    print(f"\nConfiguration:")
    ...
    try:
        ...
        print(f"\nTest traffic with:")
        print(f"  python payload_test_model_script.py {output_name} <test.pcap>")
