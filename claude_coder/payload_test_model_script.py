"""
payload_test_model_script.py
Test a trained PayloadMAE model against a decrypted PCAP and report anomalous flows.
...
Usage:
    python payload_test_model_script.py <model_name> <test_decrypted.pcap> [options]

Examples:
    python payload_test_model_script.py payload_baseline suspect_decrypted.pcap
    python payload_test_model_script.py payload_baseline suspect_decrypted.pcap --verbose --plot
    python payload_test_model_script.py payload_baseline suspect_decrypted.pcap --save-results out.json
"""

import argparse
import json
import sys
from payload_mae_lib import PayloadMAE, _key_to_str


def main():
    parser = argparse.ArgumentParser(
        description='Test a trained PayloadMAE model against a decrypted PCAP file'
    )
    parser.add_argument(
        'model_name',
    ...
    parser.add_argument(
        'pcap_file',
        help='Path to decrypted PCAP file to analyse (benign or malicious)'
    )
    parser.add_argument(
        '--threshold',
        type=float,
    ...
    print("=" * 70)
    print("PayloadMAE — Analyse Traffic")
    print("=" * 70)
    print(f"\nConfiguration:")
